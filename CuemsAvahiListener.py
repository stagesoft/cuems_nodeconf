...
class CuemsAvahiListener():
    ...
    def update_service(self, zeroconf, type, name):
        info = zeroconf.get_service_info(type, name)
        self.nodes[self.get_host(name)] = { 'name' : self.get_host(name), 'node_type': info.properties[list(info.properties.keys())[0]].decode("utf-8"), 'ip' : info.parsed_addresses()[0], 'port': info.port}
