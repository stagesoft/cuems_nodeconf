...
class CuemsAvahiListener():
    ...
    def add_service(self, zeroconf, type_, name):
        ...
        node = CuemsNode({ 'uuid' : info.properties[b"uuid"].decode("utf-8"), 'mac' : self.get_mac(name), 'name' : name, 'node_type': CuemsNode.NodeType[info.properties[b'node_type'].decode("utf-8")] , 'ip' : ip, 'port': info.port, 'online': True, 'adopted': False})
        try:
    ...
    def update_service(self, zeroconf, type_, name):
        ...
        if len(info.parsed_addresses()) > 1:
            ...
            ip = info.parsed_addresses()[0] 
        node = CuemsNode({ 'uuid' : info.properties[b"uuid"].decode("utf-8"), 'name' : name, 'node_type': CuemsNode.NodeType[info.properties[list(info.properties.keys())[0]].decode("utf-8")], 'ip' : ip, 'port': info.port, 'online': True, 'adopted': False})
        self.nodes[self.get_mac(name)].update(node)
